...
fn main() {
  ...
  for i in 0..board.len() {
    answer = max(answer, energized_from_starting_position((i, 0, Right), &board));
    answer = max(answer, energized_from_starting_position((i, board[0].len()-1, Left), &board));
  }
  for i in 0..board[0].len() {
    answer = max(answer, energized_from_starting_position((0, i, Down), &board));
    answer = max(answer, energized_from_starting_position((board.len()-1, i, Up), &board));
  }
  println!("{}", answer);
}