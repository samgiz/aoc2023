...
impl Brick {
  fn overlaps_with(&self, other: &Brick) -> bool {
    ((self.x1 <= other.x1 && other.x1 <= self.x2) || (other.x1 <= self.x1 && self.x1 <= other.x2))
      && ((self.y1 <= other.y1 && other.y1 <= self.y2)
        || (other.y1 <= self.y1 && self.y1 <= other.y2))
  }
}

fn main() {
  let mut bricks = io::stdin()
    .lines()
    .map(|line| {
      let line = line.unwrap();
      let [xyz1, xyz2]: [&str; 2] = line.split('~').collect::<Vec<_>>().try_into().unwrap();
      let [x1, y1, z1]: [i64; 3] = xyz1
        .split(',')
        .map(|x| x.parse::<i64>().unwrap())
        .collect::<Vec<_>>()
        .try_into()
        .unwrap();
      let [x2, y2, z2]: [i64; 3] = xyz2
        .split(',')
        .map(|x| x.parse::<i64>().unwrap())
        .collect::<Vec<_>>()
        .try_into()
