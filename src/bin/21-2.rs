  ...
  while !to_visit.is_empty() {
    ...
    for (drow, dcol) in [(0, 1), (0, -1), (1, 0), (-1, 0)] {
      ...
      if (map[row][col] == b'.' || map[row][col] == b'S') && !visited.contains_key(&(row, col)) {
        ...
        // dbg!("not added", map[row][col], visited.contains_key(&(row, col)));
      }
    }
  }
  // find max number of steps
  let max_num_steps = *visited.iter().map(|(_, steps)| steps).max().unwrap();
  let mut answers = vec!(0 as u64; (max_num_steps+1) as usize);
  // dbg!(visited.clone());
...
fn answer_horizontally(amount: u64, num_reachable: &Vec<u64>, map: &Vec<Vec<u8>>, total_reachable_in_block: u64) -> u64 {
  let mut answer = 0;
  // move as far as we can down
...
fn main() {
  ...
  let num_reachable = total_reachable(even_reachable, odd_reachable, 0 + (map.len() - starting_position.0), starting_position.1);
  // dbg!(answer_vertically(amount, &from_top_center, &map, num_reachable));
  ...
  // dbg!(num_reachable);
  // dbg!(from_bottom_center.clone());
  // dbg!(answer_vertically(amount, &from_bottom_center, &map, num_reachable));
  ...
  let num_reachable = total_reachable(even_reachable, odd_reachable, 0 + (map.len() - starting_position.0) + (starting_position.1 + 1), map[0].len() - 1);
  ...
  let num_reachable = total_reachable(even_reachable, odd_reachable, 0 + (map.len() - starting_position.0) + (map[0].len() - starting_position.1), 0);
