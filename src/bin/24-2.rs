...
fn main() {
  ...
  io::stdin().lines().for_each(|line| {
    ...
    let [px, py, pz]: [f64; 3] = p
      .split(", ")
      .map(|x| x.trim().parse::<f64>().unwrap())
      .collect::<Vec<_>>()
      .try_into()
      .unwrap();
    let [vx, vy, vz]: [f64; 3] = v
      .split(", ")
      .map(|x| x.trim().parse::<f64>().unwrap())
      .collect::<Vec<_>>()
      .try_into()
  ...
  let b: Array1<f64> = array![
    -(p_i[0][2] * v_i[0][1] - p_i[0][1] * v_i[0][2] + p_i[1][1] * v_i[1][2]
      - p_i[1][2] * v_i[1][1]),
    -(p_i[0][0] * v_i[0][2] - p_i[0][2] * v_i[0][0] + p_i[1][2] * v_i[1][0]
      - p_i[1][0] * v_i[1][2]),
    -(p_i[0][1] * v_i[0][0] - p_i[0][0] * v_i[0][1] + p_i[1][0] * v_i[1][1]
      - p_i[1][1] * v_i[1][0]),
    -(p_i[0][2] * v_i[0][1] - p_i[0][1] * v_i[0][2] + p_i[2][1] * v_i[2][2]
      - p_i[2][2] * v_i[2][1]),
    -(p_i[0][0] * v_i[0][2] - p_i[0][2] * v_i[0][0] + p_i[2][2] * v_i[2][0]
      - p_i[2][0] * v_i[2][2]),
    -(p_i[0][1] * v_i[0][0] - p_i[0][0] * v_i[0][1] + p_i[2][0] * v_i[2][1]
      - p_i[2][1] * v_i[2][0]),
  ];
  dbg!(a.clone());
  dbg!(b.clone());
