...
fn main() {
  ...
  for line in lines {
    ...
    let [mut record, sizes]: [String; 2] = line.split(' ').map(|x|x.to_string()).collect::<Vec<String>>().try_into().unwrap();
    record.push('?');
    let mut record = record.repeat(5);
    record.pop();
    record.push('.');
    let record = record.as_bytes();
    let sizes: Vec<_> = sizes.split(',').map(|x|x.parse::<u64>().unwrap()).collect();
