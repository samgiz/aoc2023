...
fn main() {
  ...
  loop {
    ...
    while !to_visit.is_empty() {
      ...
      if to_label == "rx" && pulse == Low {
        println!("{}", num_presses);
        std::process::exit(0);
      } else if to_label == "rg" && pulse == High {
        // Figure out the period from this and multiply the periods together, that seems to give the correct answer.
        // Not a hundred percent sure how to do this properly just yet.
        dbg!(from_label.clone(), num_presses);
      }
